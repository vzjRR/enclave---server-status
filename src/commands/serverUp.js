...
const data = new SlashCommandBuilder()
  .setName('server-up')
  .setDescription('Announce the server is back up to the status channel (with @everyone).')
...
async function execute(interaction) {
  ...
  const sent = await statusWatcher.postStatusMessage(interaction.client, embeds.serverUp(), 'server-up');
